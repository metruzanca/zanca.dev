...
- **GitHub data** (pinned repos, all repos, contribution graph) is fetched at build time into `data/github.snapshot.json` (regenerated by `pnpm build`, not committed) and refreshed at runtime every 6h via the GitHub GraphQL API.
